...
const installHefangCrossPageIntroRouting = () => {
    ...
    document.addEventListener('click', (event) => {
        ...
        try { sessionStorage.setItem(HEFANG_INTRO_TARGET_KEY, targetPage); } catch (error) {}
        try { window.name = `hefang-intro:${targetPage}`; } catch (error) {}

        const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
        const transition = document.querySelector('.page-transition-liquid');
        if (reduceMotion || !transition || !window.gsap) return;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (heroBgVideo) {
        ...
        const revealAfterDrawableFrame = () => {
            if ('requestVideoFrameCallback' in HTMLVideoElement.prototype) {
                heroBgVideo.requestVideoFrameCallback(revealHeroVideoFrame);
            } else {
    ...
    if (transitionBg && !window.__hefangPlayPageIntro) {
        transitionBg.style.display = 'none';
    } else if (transitionBg && typeof gsap !== 'undefined') {
        let pageHasRevealed = false;
        let runtimeReady = document.documentElement.classList.contains('hero-runtime-ready');
        let videoReady = Boolean(window.__hefangHeroVideoReady);

        gsap.set(transitionBg, {
            scale: 1.5,
            autoAlpha: 1,
            force3D: true,
            transformOrigin: '50% 50%'
        ...
        const revealPreparedHero = () => {
            if (pageHasRevealed || !runtimeReady || !videoReady) return;
            pageHasRevealed = true;
                ...
                requestAnimationFrame(() => {
                    window.setTimeout(() => {
                        gsap.to(transitionBg, {
                            ...
                            ease: 'power3.inOut',
                            force3D: true,
                            onComplete: finishHomePageIntro
                        });
                    }, 60);
                });
            });
        ...
        window.addEventListener('hefang:hero-runtime-ready', () => {
            runtimeReady = true;
            revealPreparedHero();
        }, { once: true });
        window.addEventListener('hefang:hero-video-ready', () => {
            videoReady = true;
            revealPreparedHero();
        }, { once: true });
    ...
    } else if (transitionBg) {
        transitionBg.style.display = 'none';
        document.documentElement.classList.add('hero-runtime-ready');
        finishHomePageIntro();
        if (typeof window.__hefangReleaseBootLock === 'function') window.__hefangReleaseBootLock();
    }
});
...
function runHefang3DStage() {
    ...
    function animate(now = 0) {
        ...
        if (!stageIsVisible || document.hidden || bottleRenderOccluded || now - lastFrameTime < frameInterval) return;
        lastFrameTime = now;
