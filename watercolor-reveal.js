(() => {
    ...
    const bootWatercolors = () => {
        if (watercolorBooted) return;
        watercolorBooted = true;
        initWatercolorReveals();
    };
    const scheduleWatercolors = () => {
        if (window.__hefangPlayPageIntro) {
            window.addEventListener('hefang:page-intro-complete', bootWatercolors, { once: true });
            window.setTimeout(bootWatercolors, 1800);
        } else {
            bootWatercolors();
        }
    };
